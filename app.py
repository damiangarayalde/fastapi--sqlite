...
@app.put("/character/update/{id}", response_model=Character)
def update_character_by_id(id: int, character: Character, db: Session = Depends(get_db)):
    ...
    if character_to_update is None:
        raise HTTPException(
            status_code=404,
            detail=f"There is no record with ID: {id}"
        )
